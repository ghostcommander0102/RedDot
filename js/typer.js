...
var scrollToElement = function (el, flag) {
    if (animateFlag == true) return;
    var speed = 2400;

    var pos = $(el)[0].offsetTop;
    if (pos != 0)
        pos -= (window.innerHeight - $(el).height()) / 2;
    console.log(pos);
    $('html,body').animate({
        scrollTop: pos
    }, speed, "swing");
    ...
    if (flag == 0) {
        $(".content-sections").css("transform", "skewY(2.0deg)")
        clearTimeout(skewTimer);
        skewTimer = setTimeout(function () {
            $(".content-sections").css("transform", "initial")
        }, 1000);
    } else {
        $(".content-sections").css("transform", "skewY(-2.0deg)")
        clearTimeout(skewTimer);
...
$(document).ready(function () {
    ...
    $("#header")[0].addEventListener("wheel", function (e) {
        ...
        if (checkScrollDirectionIsUp(e)) {
            ...
            scrollToElement($(".project-section"), 1);
            setTimeout(function () {
                $(".project.project-default.wrap").addClass("active");
            }, 2000);
        }
    });
    $(".project-section")[0].addEventListener("wheel", function (e) {
        console.log("Project");
        if (checkScrollDirectionIsUp(e)) {
            scrollToElement($("#header"), 0);
        } else {
            if (animateFlag == false) {
                $("#red_back").fadeOut(2000);
                $("#soft_blurback").fadeTo(0, 0.1).fadeTo(2000, 1);
            }
            scrollToElement($(".branding-section"), 1);
            setTimeout(function () {
                $(".branding-section.wrap").addClass("active");
            }, 2000);
        }
    });
    $(".branding-section")[0].addEventListener("wheel", function (e) {
        console.log("Branding");
        if (checkScrollDirectionIsUp(e)) {
            if (animateFlag == false) {
                $("#red_back").fadeIn(2000);
                $("#soft_blurback").fadeOut(2000);
            }
            scrollToElement($(".project-section"), 0);
        } else {
            scrollToElement($(".pricing-section"), 1);
            setTimeout(function () {
                $(".pricing-section.wrap").addClass("active");
            }, 2000);
        }
    });
    $(".pricing-section")[0].addEventListener("wheel", function (e) {
        if (checkScrollDirectionIsUp(e)) {
            scrollToElement($(".branding-section"), 0);
        } else {
        }
    });
})
